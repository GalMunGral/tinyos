...
static inline void       *pa2kva(uint64_t pa)  { return (void *)(pa + KERNEL_OFFSET); }
static inline uint64_t    kva2pa(void *va)     { return (uint64_t)va - KERNEL_OFFSET; }

static inline uint64_t pt2satp(pte_t *pt) { return (8UL << 60) | (kva2pa(pt) >> 12); }
...
void        free_pagetable(pte_t *pt);  // recursively free all mapped pages and page table nodes